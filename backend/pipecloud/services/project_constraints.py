...
def project_constraints_payload(project):
    ...
    for rule_key, definition in CONSTRAINT_DEFINITIONS.items():
        ...
        rules.append({
            'key': rule_key,
            'enabled': bool(constraint and constraint.enabled),
            'parameters': dict(constraint.parameters or {}) if constraint else {},
...
def update_project_constraints(project, rules):
    ...
    for rule in rules:
        ...
        if enabled and value not in definition['options']:
        ...
        ProjectConstraint.objects.update_or_create(
            ...
            defaults={
                'enabled': enabled,
                'parameters': {parameter_name: value} if enabled else {},
            },
        )
