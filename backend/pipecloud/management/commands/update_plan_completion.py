...
class Command(BaseCommand):
    help = '同步当日焊接计划完成情况，并将未完成焊口滚动到后续计划日期'
    ...
    def handle(self, *args, **options):
        ...
        if not results:
            self.stdout.write(self.style.WARNING('没有可处理的项目'))
            return

        failed = 0
        for result in results:
            project_label = f'[{result["projectId"]}] {result["projectName"]}'
            if result.get('error'):
                failed += 1
                self.stderr.write(self.style.ERROR(f'{project_label} 失败：{result["error"]}'))
                continue
            if result.get('skipped'):
                self.stdout.write(self.style.WARNING(f'{project_label} 已跳过：{result.get("reason", "")}'))
                continue
            self.stdout.write(self.style.SUCCESS(
                f'{project_label} 当日 {result.get("todayWeldCount", 0)} 道，'
                f'完成 {result.get("completedWeldCount", 0)} 道，'
                f'滚动 {result.get("rolledWeldCount", 0)} 道，'
