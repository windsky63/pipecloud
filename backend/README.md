...
定时任务使用独立进程运行，默认每天 21:00（Asia/Shanghai）更新焊接计划完成情况，
并将未完成焊口滚动到后续计划：
...
可在部署系统中将 Web 服务和上述命令配置为两个独立的常驻进程。手动预演或补跑：
...
执行时间可通过 `PIPECLOUD_PLAN_ROLLOVER_HOUR`、`PIPECLOUD_PLAN_ROLLOVER_MINUTE`
和 `PIPECLOUD_PLAN_ROLLOVER_MISFIRE_GRACE_SECONDS` 调整。
