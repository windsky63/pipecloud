...
from initialization.init_config import AUTO_WELD_FILTERS, COLUMNS, FILES, PREFAB_WELD_FILTERS
...
def main():
    ...
    if success:
        print(f'initialization完成，可预制输出文件: {FILES["prefab_filtered_output"]}')
        print(f'initialization完成，自动焊输出文件: {FILES["filtered_output"]}')
    else:
        print('initialization失败')
