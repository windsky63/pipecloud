...
from 初始化文件处理.init_config import COLUMNS, DEFAULT_WELD_PRIORITY, EXTRA_MATERIAL_QTY_FOR_P, FILES, VERBOSE
...
def build_unified_weld_library(prefab_df, auto_df):
    ...
    if seq_col in library_df.columns:
        library_df = library_df.drop(columns=[seq_col])
    library_df.insert(0, seq_col, range(1, len(library_df) + 1))
