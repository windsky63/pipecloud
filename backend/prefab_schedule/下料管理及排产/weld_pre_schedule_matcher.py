...
def _aggregate_fitting_demands(fitting_demands):
    ...
    grouped = (
        demand_df.groupby(MATERIAL_CODE_COL, as_index=False)
        .agg({
...
def _prepare_uncompleted_welds(weld_df):
    ...
    weld_df[seq_col] = pd.to_numeric(weld_df[seq_col], errors='coerce').fillna(0).astype(int)

    uncompleted_df = weld_df.loc[~weld_df[completed_col]].copy()
    if PRE_SCHEDULE_ONLY_AUTO_WELD:
    ...
    return uncompleted_df.sort_values([priority_col, seq_col], ascending=[False, True], kind='mergesort').reset_index(drop=True)
...
def match_weld_pre_schedule(
    ...
    pipe_df, _ = cutting_main.ensure_anti_corrosion_pipe_library(
        anti_corrosion_pipe_library_file=anti_corrosion_pipe_library_file,
        persist_initialized=False,
    )
    fitting_df, _ = cutting_main.ensure_anti_corrosion_fitting_library(
        anti_corrosion_fitting_library_file=anti_corrosion_fitting_library_file,
        persist_initialized=False,
    )

    pipe_df = cutting_main._normalize_pipe_library(pipe_df)
    fitting_df = cutting_main._normalize_fitting_library(fitting_df)
    pipe_states = _build_pipe_states(pipe_df)
    fitting_stock = _build_fitting_stock(fitting_df)
    candidate_df = _prepare_uncompleted_welds(weld_df)
    ...
    for _, group_df in candidate_df.groupby(group_cols, sort=False, dropna=False):
        group_result = _simulate_group_matches(group_df, pipe_states, fitting_stock, match_seq)
        detail_rows.extend(group_result['detail_rows'])

        if _group_should_be_rejected(group_result):
            group_rejected_rows, group_all_rows = _reject_group_rows(group_result)
    ...
    updated_pipe_df = _apply_pipe_states_to_df(pipe_df, pipe_states)
    prepare_output_file(pending_anti_corrosion_pipe_library_file)
    updated_pipe_df.to_excel(pending_anti_corrosion_pipe_library_file, index=False)

    updated_fitting_df = _apply_fitting_stock_to_df(fitting_df, fitting_stock)
    prepare_output_file(pending_anti_corrosion_fitting_library_file)
    updated_fitting_df.to_excel(pending_anti_corrosion_fitting_library_file, index=False)
