...
- 项目隔离：新建项目后在 MySQL 中创建项目记录，初始化数据、材料库、解析结果、排产结果、计划摘要及其他结构化记录通过项目标识关联和隔离。
- 文件数据：上传文件、解析产物、业务 Excel、导出包和备份仍保存在后端受控运行目录中，但系统不再以“每个项目一个独立后端文件夹”作为主要结构化数据存储方案。
- 访问约束：除项目首页、预制工厂、系统设置和开发者控制等少数页面外，核心业务页面必须先选择当前项目。

## 功能模块

### 1. 项目首页

项目首页是项目级总控台，集成项目切换、项目数据、约束配置、业务看板和工作流入口。
